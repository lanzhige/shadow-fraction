#ifndef CALCULATEMRT_COMPRESSION_H_
#define CALCULATEMRT_COMPRESSION_H_
...
namespace file {
	...
	static int compress(unsigned char *source, const string &outpath, int size, int level = Z_DEFAULT_COMPRESSION)
	{
		int ret, flush;
		unsigned have;
		z_stream strm;
	...
	static int decompress(const string& inpath, unsigned char *out, int size) {
		int ret;
		unsigned have;
		z_stream strm;
		unsigned char *in;
...
#endif // !CALCULATEMRT_COMPRESSION_H_
